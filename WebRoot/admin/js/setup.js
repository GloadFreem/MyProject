...
function drawPointsChart(containerElement) {
	...
	var plot3 = $.jqplot(containerElement, [ cosPoints, sinPoints, powPoints1,
			powPoints2 ], {
...
function drawBubbleChart(containerElement) {
	...
	var arr = [ [ 11, 123, 1236, "" ], [ 45, 92, 1067, "" ],
			[ 24, 104, 1176, "" ], [ 50, 23, 610, "A" ], [ 18, 17, 539, "" ],
			[ 7, 89, 864, "" ], [ 2, 13, 1026, "" ] ];
	...
	$('#' + containerElement)
			.bind(
					...
					function(ev, seriesIndex, pointIndex, data, radius) {
						var chart_left = $('#' + containerElement).offset().left, chart_top = $(
								'#' + containerElement).offset().top, x = plot1b.axes.xaxis
								.u2p(data[0]), // convert x axis unita to
						// pixels
						y = plot1b.axes.yaxis.u2p(data[1]); // convert y axis
						// units to pixels
						...
						$('#legend1b tr').eq(pointIndex + 1).css(
	...
	$('#' + containerElement).bind(
			...
			function(ev, seriesIndex, pointIndex, data) {
				$('#tooltip1b').empty();
				$('#tooltip1b').hide();
				$('#' + containerElement + ' tr').css('background-color',
						'#ffffff');
			});
}
...
function setupGallery() {
	...
	$('ul.gallery').each(
			function() {
				...
				var $gallerySorting = $(this).parent().prev().children(
						'ul.sorting');
				...
				// item is clicked
				$($gallerySorting).find("a[data-type]").click(
						function(e) {
							// reset the active class on all the buttons
							$($gallerySorting).find("a[data-type].active")
									.removeClass('active');
							...
							if ($filterType == 'all') {
								// assign all li items to the $filteredData var
								// when
...
function setupLeftMenu() {
	...
	// data.newHeader.next().andSelf().css('display', 'inherit');
	// data.oldHeader.next().andSelf().css('display', 'inherit');
	// data.newHeader.next().andSelf().addClass("current");
	// data.oldHeader.next().andSelf().removeClass("current");
	// }).find("a.menuitem:first").next().css('display', 'none');
...
//		"header" : "a.menuitem"
//	}).bind("accordionchangestart", function(e, data) {
//		data.newHeader.next().andSelf().css("visibility", "visible");
//		data.oldHeader.next().andSelf().css("visibility", "visible");
//		data.newHeader.next().andSelf().next().css("visibility", "visible");
//		data.oldHeader.next().andSelf().next().css("visibility", "visible");
//		data.newHeader.next().andSelf().addClass("current");
//		data.oldHeader.next().andSelf().removeClass("current");
//	}).find("a.menuitem:first").next().css("visibility", "hidden");
//	$("#section-menu").accordion({
//		"header" : "a.menuitem"
//	}).bind("accordionchangestart", function(e, data) {
//	}).find("a.menuitem:first").next().css("visibility", "hidden");
	
	$(".menuitem").next().css("visibility", "hidden");
