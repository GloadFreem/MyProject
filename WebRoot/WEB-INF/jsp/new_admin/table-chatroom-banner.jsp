								...
								<c:when test="${result != null}">
									<c:forEach items="${result}" var="item" varStatus="status">
										<tr>
											...
											<td>${item.affiliationsCount}</td>
											<td>${item.maxusers}</td>
											<td>所属项目</td>
											<td>${item.ownerName}</td>
											<td>${item.createDate}</td>
											<td><a href="createChatRoomPage.action?contentId=${item.chatroomId }" class="active"><i
													class="fa fa-edit text-success text-active"></i><i
													class="fa fa-edit text-danger text"></i></a> | <a
												href="#modal" data-href="deleteChatRoom.action?contentId=${item.chatroomId }" data-toggle="modal"
												class="active"><i
