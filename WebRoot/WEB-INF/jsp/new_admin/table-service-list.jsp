	...
	<section class="vbox">
		<section class="scrollable padder">
			<ul class="breadcrumb no-border no-radius b-b b-light pull-in">
				...
				<li><a href="index.action"><i class="fa"></i> 服务列表</a></li>
			</ul>
								...
								<c:when test="${result != null}">
									<c:forEach items="${result}" var="item" varStatus="status">
										<tr>
											...
											<td>${item.typeId}</td>
											<td>${item.name}</td>
											<td width="100"><a href="${item.image}" target="blank"><img
													src="${item.image}" alt="城投逸园" class="img-responsive" /></a></td>
											<td>${item.paytype.name}</td>
											<td>${item.price}</td>
											<td>${item.content}</td>
											<td><a
												href="serviceDetail.action?contentId=${item.typeId }&menu=2&sortmenu=1&submenu=1&page=0&size=10"
												class="active"><i
													class="fa fa-edit text-success text-active"></i><i
													class="fa fa-edit text-danger text"></i></a> | <a href="#modal"
												data-href="deleteService.action?contentId=${item.typeId }&menu=2&sortmenu=1&submenu=1&page=0&size=10"
...
<script type="text/javascript">
	$("a[name='page']").click(
			function() {
				...
				var submenu = ${submenu};
				var url = "serviceList.action?" + "size=" + size + "&page=" + page
