								...
								<c:when test="${result != null}">
									<c:forEach items="${result}" var="item" varStatus="status">
										<tr>
											...
											<td>${item.content}</td>
											<td><img
												src="genErCode.action?url='requestCompleteTask.action?taskId=${item.taskId}'&width=100&height=100"
												alt="任务码" class="img-responsive" /> <!-- 				<img src="genErCode.action?url='requestUserAttendWork.action'&width=100&height=100" alt="任务码"
													class="img-responsive" /> --></td>
											<td>${item.taskDate}</td>
											<td><a
												href="taskDetail.action?contentId=${item.taskId }&menu=6&sortmenu=1&submenu=1&page=0&size=10"
												class="active"><i
													class="fa fa-edit text-success text-active"></i><i
													class="fa fa-edit text-danger text"></i></a> | <a href="#modal"
												data-href="deleteTask.action?contentId=${item.taskId }&menu=6&sortmenu=1&submenu=1&page=0&size=10"
...
<script type="text/javascript">
	$("a[name='page']").click(
			function() {
				...
				var submenu = ${submenu};
				var url = "taskList.action?" + "size=" + size + "&page=" + page
