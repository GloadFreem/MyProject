								...
								<c:when test="${result != null}">
									<c:forEach items="${result}" var="item" varStatus="status">
										<tr>
											...
											<td>${item.service.servicetype.price}元</td>
											<td>${item.orderDate}</td>
											<td><a
												href="orderDetail.action?contentId=${item.orderId }&menu=3&sortmenu=1&submenu=1&page=0&size=10"
												class="active"><i
													class="fa fa-edit text-success text-active"></i><i
													class="fa fa-edit text-danger text"></i></a> | <a href="#modal"
												data-href="deleteOrder.action?contentId=${item.orderId }&menu=3&sortmenu=1&submenu=1&page=0&size=10"
...
<script type="text/javascript">
	$("a[name='page']").click(
			function() {
				...
				var submenu = ${submenu};
				var url = "orderList.action?" + "size=" + size + "&page="
